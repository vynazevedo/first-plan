...
//!
//! **Plataforma:** apenas Unix (Linux, macOS). Em Windows, os symbols publicos
//! existem mas retornam erros - daemon mode fica para v0.7+ (named pipes).
...
#[cfg(unix)]
pub use unix_impl::{is_running, send_request, status, stop, Daemon};

#[cfg(not(unix))]
pub use stub_impl::{is_running, send_request, status, stop, Daemon};

#[cfg(unix)]
mod unix_impl {
    use super::{pid_path, read_pid, socket_path, DaemonRequest, DaemonResponse, DaemonStatus};
    use crate::lsp::client::LspClient;
    use crate::lsp::ops;
    use crate::lsp::registry::{server_for_path, ServerId};
        ...
        match req.op.as_str() {
            "status" => {
                let warm = daemon
                    ...
                    .await
                    .keys()
                    .map(|id| crate::lsp::registry::spec(*id).name.to_string())
                    .collect::<Vec<_>>();
            ...
            "wsymbols" => {
                ...
                let sid = match server_name {
                    Some(name) => ServerId::all()
                        .iter()
                        .copied()
                        .find(|id| crate::lsp::registry::spec(*id).name == name)
                        .ok_or_else(|| anyhow!("unknown server name: {}", name))?,
                    None => crate::lsp::registry::servers_for_project(&daemon.root)
                        .into_iter()
                        .next()
...
#[cfg(not(unix))]
mod stub_impl {
    use super::{pid_path, socket_path, DaemonRequest, DaemonStatus};
    ...
    pub async fn send_request(_req: &DaemonRequest) -> Result<Value> {
        Err(anyhow!(
            "daemon mode requires Unix sockets (Linux/macOS). Windows support tracked for v0.8+ via named pipes."
        ))
    }
